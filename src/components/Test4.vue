<template>
  <div class="table-wrap coltype" style="width: 50%">
    ...
    <input v-model="search" type="text" class="form-control m-0" >
    <table class="table ui-accordion">
      ...
      <tbody>
        ...
        <tr class="head" :key="key" v-for="(row, key) in rows">
          <td>{{row.order}}</td>
          <td>{{row.type}}</td>
          <td>{{row.question}}</td>
        </tr>
  ...
  computed: {
    rows () {
      return this.data.rows.filter((row) => {
        return row.order.toString().includes(this.search.toLowerCase()) || row.type.toLowerCase().includes(this.search.toLowerCase()) || row.question.toLowerCase().includes(this.search.toLowerCase())
      })
    }
  }
}
